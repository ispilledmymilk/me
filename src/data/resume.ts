...
export const resume: Resume = {
  ...
  projects: [
    {
      ...
      dates: 'Jul 2026',
      stackByCategory: [
        { label: 'Languages', items: ['Scala', 'Python', 'SQL'] },
        {
          label: 'Streaming & processing',
          items: ['Kafka', 'Spark Structured Streaming', 'PySpark', 'Databricks'],
        },
        {
          label: 'Warehouse & storage',
          items: ['Snowflake', 'Snowpipe', 'GCS', 'Parquet'],
        },
        {
          label: 'Orchestration & IaC',
          items: ['Airflow', 'Cloud Composer', 'Terraform', 'GCP'],
        },
        {
          label: 'Observability & delivery',
          items: ['Prometheus', 'Grafana', 'Docker', 'GitHub Actions', 'Testcontainers'],
        },
      ],
      bullets: [
        'Built a distributed real-time pipeline that watches prediction-market order books (bids, asks, trades, resolutions) and flags suspicious wallet patterns — e.g. dumping a huge position seconds before a market resolves.',
        'Ingestion: Scala producer simulates (or later connects to) a live order book and pushes events into Kafka; Spark Structured Streaming cleans the stream, aggregates into 1-minute windows, and applies unit-tested anomaly rules.',
        'Landing & warehouse path: writes cleaned trades and anomaly results as Parquet (GCS in cloud / local file:// in CI); Snowflake + Snowpipe scaffolded for analyst marts when accounts are configured.',
        'Ops layer: Airflow (Cloud Composer) kicks and monitors the Spark/Databricks job with failure alerts; Prometheus + Grafana surface Kafka lag and throughput so pipeline health is visible.',
        'Runnable for free today: Docker Compose local demo (fake trades → Kafka → Grafana metrics) plus Make-driven suites — Scala unit/IT (Testcontainers), pytest anomaly rules, and local PySpark Parquet e2e; Terraform scaffolds GCP Composer/Databricks/GCS for optional cloud deploy.',
      ],
    },
    {
      name: 'AndyDandy',
  ...
  skills: [
    ...
    {
      label: 'Data engineering & streaming',
      items: [
        'Kafka',
